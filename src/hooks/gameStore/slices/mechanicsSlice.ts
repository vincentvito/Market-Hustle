...
import {
  ...
  QUIET_NEWS,
  LIFESTYLE_EFFECTS,
  expandLifestyleEffects,
  rollForPEExit,
  ASSET_EVENT_MULTIPLIERS,
...
import {
  DEFAULT_GOSSIP_STATE,
  shouldShowGossip,
  createGossipNewsItem,
  isMarketSideways,
  hasMajorEventToday,
} from "@/lib/game/gossip";
...
import {
  getScriptedGame,
  getScriptedGameNumber,
} from "@/lib/game/scriptedGames";
import {
  getRemainingGames,
  getLimitType,
  generateGameId,
...
async function submitScore(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `Score submit attempt ${attempt}/${retries} failed: ${res.status}`,
      );
    } catch (err) {
...
function saveGameResult(
  ...
  const entry: GameHistoryEntry = {
    ...
    outcome: isWin
      ? "win"
      : mapGameOverReasonToOutcome(gameOverReason || "BANKRUPT"),
  };
...
export const createMechanicsSlice: MechanicsSliceCreator = (set, get) => ({
  ...
  showPortfolioBeforeAdvance:
    typeof window !== "undefined"
      ? localStorage.getItem("showPortfolioBeforeAdvance") === "true"
      : false,
  ...
  operationStates: {
    lobby_congress: {
      operationId: "lobby_congress",
      lastUsedDay: null,
      timesUsed: 0,
    },
    execute_coup: {
      operationId: "execute_coup",
      lastUsedDay: null,
      timesUsed: 0,
    },
    plant_story: {
      operationId: "plant_story",
      lastUsedDay: null,
      timesUsed: 0,
    },
  },
  ...
  startGame: (
    duration?: GameDuration,
    options?: { cash?: number; debt?: number; skipLimits?: boolean },
  ) => {
    const userState = loadUserState();
    ...
    const beginGame = (remaining?: number) => {
      ...
      if (activeScript) {
        ...
        if (day1.flavorHeadline) {
          const flavorLabel = day1.flavorHeadline.startsWith("@")
            ? ("gossip" as const)
            : detectLabelType(day1.flavorHeadline, "flavor");
          todayNewsItems.push({
            headline: day1.flavorHeadline,
            effects: {},
            labelType: flavorLabel,
          });
        }
        ...
        todayNewsItems = openingEvent
          ? [
              {
                headline: openingEvent.headline,
                effects: openingEvent.effects,
                labelType: "news",
              },
            ]
          : [
              {
                headline: "MARKETS OPEN - GOOD LUCK TRADER",
                effects: {},
                labelType: "news",
              },
            ];
        ...
        priceHistory[asset.id] = [
          {
            ...
            low:
              Math.round(Math.max(0.01, Math.min(low, open, close)) * 100) /
              100,
            close: close,
          },
        ];
      ...
      set({
        ...
        buyQty: 1,
        newsHistory: [
          openingEvent?.headline || "MARKETS OPEN - GOOD LUCK TRADER",
        ],
        showPortfolio: false,
        portfolioAdvancePending: false,
        activeChains: [],
      ...
      try {
        const playLog: string[] = JSON.parse(
          localStorage.getItem("mh_play_log") || "[]",
        );
        playLog.push(new Date().toISOString());
        // Keep last 500 entries
        localStorage.setItem(
          "mh_play_log",
          JSON.stringify(playLog.slice(-500)),
        );
      } catch {
        /* silent */
      }
    ...
    if (!skipLimits && !isDev && !storeIsLoggedIn) {
      // Race-condition guard: TierSync may not have set isLoggedIn yet.
      // Do a quick Supabase session check before treating user as a guest.
      const { getSupabaseClient } = require("@/lib/supabase/client");
      const supabase = getSupabaseClient();
      supabase.auth.getSession().then(({ data }: { data: { session: unknown } }) => {
        if (data.session) {
          // User IS logged in — TierSync just hasn't synced yet.
          // Update store and take the logged-in path.
          set({ isLoggedIn: true });
        ...
        // If session check fails, fall back to guest path
        fetch("/api/game/guest-start", { method: "POST" })
  ...
  nextDay: () => {
    const {
      ...
      pendingAbilityEffects,
      pendingStoryArc,
      pendingPhase2Effects,
      usedPEAbilities,
      pendingInflationEffect,
      directorState,
      directorConfig,
      gameDuration,
      initialNetWorth,
    ...
    let bestRippleCandidate: {
      headline: string;
      category: string;
      effects: Record<string, number>;
    } | null = null;
    function updateRippleCandidate(
      headline: string,
      category: string,
      eventEffects: Record<string, number>,
    ) {
      const impact = Object.values(eventEffects).reduce(
        (sum, e) => sum + Math.abs(e),
        0,
      );
      const currentBest = bestRippleCandidate
        ? Object.values(bestRippleCandidate.effects).reduce(
            (sum, e) => sum + Math.abs(e),
            0,
          )
        : 0;
    ...
    if (pendingAbilityEffects) {
      Object.entries(pendingAbilityEffects.effects).forEach(
        ([assetId, effect]) => {
          effects[assetId] = (effects[assetId] || 0) + effect;
        },
      );

      const fallbackName = pendingAbilityEffects.abilityId
        .replace(/_/g, " ")
        .toUpperCase();
      const eventHeadline =
        pendingAbilityEffects.eventHeadline ||
        (pendingAbilityEffects.isBackfire
          ? `BACKFIRE: ${fallbackName} OPERATION EXPOSED`
          : `${fallbackName} SUCCESSFUL`);
    ...
    if (pendingStoryArc) {
      ...
      if (nextPhase === 2) {
        ...
        const labelType = isSilentExecution
          ? ("rumor" as const)
          : ("developing" as const);
        todayNewsItems.push({
        ...
        if (
          pendingStoryArc.storyOutcome === "unfavorable" &&
          pendingStoryArc.additionalConsequences
        ) {
        ...
        revealedUsedPEAbilities = revealedUsedPEAbilities.map((u) =>
          u.abilityId === pendingStoryArc.abilityId
            ? {
                ...u,
                didBackfire: pendingStoryArc.storyOutcome === "unfavorable",
              }
            : u,
        );

        if (
          pendingStoryArc.abilityId === "project_chimera" &&
          pendingStoryArc.storyOutcome === "favorable"
        ) {
    ...
    if (pendingPhase2Effects && newDay >= pendingPhase2Effects.triggerOnDay) {
      Object.entries(pendingPhase2Effects.effects).forEach(
        ([assetId, effect]) => {
          effects[assetId] = (effects[assetId] || 0) + effect;
        },
      );
    ...
    if (
      pendingInflationEffect &&
      newDay >= pendingInflationEffect.triggerOnDay
    ) {
      Object.entries(pendingInflationEffect.effects).forEach(
        ([assetId, effect]) => {
          effects[assetId] = (effects[assetId] || 0) + effect;
        },
      );
    ...
    ownedLuxury.forEach((id) => {
      ...
      if (asset && asset.dailyCost !== 0) {
        if (asset.dailyCost > 0) {
          ...
          const label =
            luxuryCostLabels[id] || `${asset.name.toUpperCase()} COSTS`;
          todayNewsItems.push({
    ...
    const ownsTenutaLuna = ownedLifestyle.some(
      (o) => o.assetId === "pe_tenuta_luna",
    );
    const ownsApexMedia = ownedLifestyle.some(
      (o) => o.assetId === "pe_apex_media",
    );
    ...
    if (ownsTenutaLuna) negativeReduction += 0.1;
    if (ownsApexMedia) negativeReduction += 0.2;
      ...
      if (scriptedDay) {
        scriptedDay.news.forEach((n) => {
          todayNewsItems.push({
            headline: n.headline,
            effects: n.effects,
            labelType: detectLabelType(n.headline, n.labelType),
          });
          Object.entries(n.effects).forEach(([assetId, effect]) => {
            effects[assetId] = (effects[assetId] || 0) + effect;
          });
        ...
        if (scriptedDay.flavorHeadline) {
          const scriptedFlavorLabel = scriptedDay.flavorHeadline.startsWith("@")
            ? ("gossip" as const)
            : detectLabelType(scriptedDay.flavorHeadline, "flavor");
          todayNewsItems.push({
            headline: scriptedDay.flavorHeadline,
            effects: {},
            labelType: scriptedFlavorLabel,
          });
        }
    ...
    if (failedPEThisDay.length > 0) {
      ...
      updatedOwnedLifestyle = updatedOwnedLifestyle.filter(
        (o) => !failedPEThisDay.includes(o.assetId),
      );
    }

    if (storyArcPEToRemove) {
      updatedOwnedLifestyle = updatedOwnedLifestyle.filter(
        (o) => o.assetId !== storyArcPEToRemove,
      );
    }
      ...
      for (const owned of shuffledPE) {
        ...
        if (exitRoll) {
          const offerAmount = Math.round(
            owned.purchasePrice * exitRoll.multiplier,
          );
          newPEExitOffer = {
          ...
          const headline = exitRoll.headline.replace(
            "{ASSET}",
            asset.name.toUpperCase(),
          );
          todayNewsItems.push({
            headline:
              exitRoll.type === "ipo" ? `🚀 ${headline}` : `💰 ${headline}`,
            effects: {},
            labelType: "breaking",
          });
    ...
    const chainsResolvingToday = updatedChains.filter(
      (chain) => chain.daysRemaining <= 1,
    );
    const resolvingChainCategories = new Set(
      chainsResolvingToday.map((c) => c.category),
    );
    ...
    updatedStories.forEach((activeStory) => {
      ...
      if (currentStageDef.branches) {
        ...
        if (outcome.continues && hasMoreStages) {
          ...
          const maxStoryEffect = Math.max(
            ...Object.values(outcome.effects).map(Math.abs),
            0,
          );
          storyLabelType = maxStoryEffect >= 0.3 ? "breaking" : "news";
        }
        todayNewsItems.push({
          headline: outcome.headline,
          effects: outcome.effects,
          labelType: storyLabelType,
        });
        hasMarketNews = true;
        updateRippleCandidate(
          outcome.headline,
          story.category,
          outcome.effects,
        );
        ...
        if (isDev) {
          console.debug(
            `[Story] ADVANCE: "${activeStory.storyId}" stage ${activeStory.currentStage}->${nextStage} — "${outcome.headline}" (label=${storyLabelType}, continues=${!!outcome.continues}, day=${newDay})`,
          );
        }
        if (outcome.continues && hasMoreStages) {
          ...
          if (isDev) {
            console.debug(
              `[Story] RESOLVED: "${activeStory.storyId}" (day=${newDay}, cooldown ${story.category} until day ${newDay + 2})`,
            );
          }
        }
      } else {
        ...
        todayNewsItems.push({
          headline: currentStageDef.headline,
          effects: currentStageDef.effects,
          labelType: "developing",
        });
        hasMarketNews = true;
        updateRippleCandidate(
          currentStageDef.headline,
          story.category,
          currentStageDef.effects,
        );

        // Record mood from story intermediate stage
        updatedAssetMoods = recordStoryMood(
          currentStageDef,
          newDay,
          updatedAssetMoods,
        );
    ...
    const storyOverdue =
      newDay >= FIRST_STORY_DAY &&
      daysSinceLastStory >= STORY_SCHEDULE_INTERVAL + 3;
    const storyDue =
      newDay >= FIRST_STORY_DAY &&
      daysSinceLastStory >= STORY_SCHEDULE_INTERVAL;
    const storyChance = storyOverdue ? 0.8 : storyDue ? 0.4 : 0.07;
    ...
    const themeCategory =
      directorState.activeTheme !== "none"
        ? themeToCategory(directorState.activeTheme)
        : null;

    if (
      !isScripted &&
      updatedStories.length < MAX_CONCURRENT_STORIES &&
      Math.random() < storyChance
    ) {
      const newStory = selectRandomStory(
        updatedUsedStoryIds,
        updatedStories,
        activeChainCategories,
        updatedAssetMoods,
        newDay,
        themeCategory,
        pendingStoryArc,
      ...
      if (newStory) {
        ...
        todayNewsItems.push({
          headline: firstStage.headline,
          effects: firstStage.effects,
          labelType: "rumor",
        });
        hasMarketNews = true;
        ...
        if (isDev) {
          console.debug(
            `[Story] STARTED: "${newStory.id}" — "${firstStage.headline}" (category=${newStory.category}, stages=${newStory.stages.length}, day=${newDay})`,
          );
        }
      }
    }
    ...
    let updatedScheduledEvent: ActiveScheduledEvent | null =
      activeScheduledEvent ? { ...activeScheduledEvent } : null;

    if (updatedScheduledEvent && !updatedScheduledEvent.resolved) {
      const scheduledDef = SCHEDULED_EVENTS.find(
        (e) => e.id === updatedScheduledEvent!.eventId,
      );
      if (scheduledDef) {
        const roll = Math.random();
        let cumulative = 0;
        let selectedOutcome =
          scheduledDef.outcomes[scheduledDef.outcomes.length - 1];
        ...
        todayNewsItems.push({
          headline: selectedOutcome.headline,
        ...
        updatedAssetMoods = recordChainOutcomeMood(
          selectedOutcome,
          newDay,
          updatedAssetMoods,
        );
    ...
    if (!isScripted && !updatedScheduledEvent && !storyFiredToday) {
      ...
      if (scheduledRoll < 0.08) {
        const scheduledBlockedCats = getActiveTopics(
          updatedStories,
          updatedChains,
        );
        ...
        if (newScheduled) {
          ...
          hasMarketNews = true;
          Object.entries(newScheduled.announcement.effects).forEach(
            ([assetId, effect]) => {
              effects[assetId] = (effects[assetId] || 0) + effect;
            },
          );
    ...
    updatedChains.forEach((chain) => {
      if (chain.daysRemaining <= 1) {
        ...
        if (isDev) {
          console.debug(
            `[Chain] TICK: "${chain.rumor}" (id=${chain.chainId}, daysRemaining=${chain.daysRemaining} -> ${chain.daysRemaining - 1}, day=${newDay})`,
          );
        }
    ...
    chainsToResolve.forEach((activeChain) => {
      ...
      if (chainDef) {
        const outcome = resolveChainWithMood(
          chainDef,
          updatedAssetMoods,
          newDay,
        );
        // Chain resolutions are narrative payoffs — lower threshold for BREAKING label
        const maxChainEffect = Math.max(
          ...Object.values(outcome.effects).map(Math.abs),
          0,
        );
        const chainLabelType = maxChainEffect >= 0.3 ? "breaking" : "news";
        todayNewsItems.push({
          headline: outcome.headline,
          effects: outcome.effects,
          labelType: chainLabelType,
        });
        hasMarketNews = true;
        updateRippleCandidate(
          outcome.headline,
        ...
        if (isDev) {
          console.debug(
            `[Chain] RESOLVED: "${activeChain.rumor}" -> "${outcome.headline}" (id=${activeChain.chainId}, day=${newDay}, label=${chainLabelType}, cooldown ${activeChain.category} until day ${newDay + 1})`,
          );
        }
      } else {
        ...
        if (isDev) {
          console.warn(
            `[Chain] RESOLUTION FAILED: chainDef not found for id="${activeChain.chainId}" (rumor="${activeChain.rumor}", day=${newDay})`,
          );
        }
        todayNewsItems.push({
    ...
    let preparedDirectorState = prepareDirectorForDay(
      directorState,
      newDay,
      gameDuration,
    );
    ...
    const directorEventBoost =
      preparedDirectorState.dopamineDebt > 0.7 ? 0.15 : 0;
    const chainProbabilityBoost =
      (directorModifiers.chainProbabilityMultiplier - 1) * 0.3;
    ...
    const baseEventChance =
      PHASE_EVENT_CHANCE[preparedDirectorState.currentPhase] ?? 0.45;

    const blockedCategories = getActiveTopics(
      updatedStories,
      updatedChains,
      pendingStoryArc,
    );
    ...
    updatedCategoryCooldowns = updatedCategoryCooldowns.filter(
      (cd) => cd.expiresDay > newDay,
    );

    let forceChain =
      directorModifiers.forceEventType === "chain" && !hasActiveChain;
    ...
    if (
      !isScripted &&
      !storyFiredToday &&
      (eventRoll < baseEventChance + directorEventBoost ||
        forceChain ||
        forceEvent)
    ) {
      const chainOrSingleRoll = Math.random();
      const adjustedChainProb = 0.5 + chainProbabilityBoost;

      if (
        !hasActiveChain &&
        (forceChain || (!forceEvent && chainOrSingleRoll < adjustedChainProb))
      ) {
        ...
        if (newChain) {
          ...
          if (isDev) {
            console.debug(
              `[Chain] STARTED: "${newChain.rumor}" (id=${newChain.id}, duration=${newChain.duration}, day=${newDay})`,
            );
          }
          // Rumor is displayed via 'rumors' state in NewsPanel, not todayNewsItems
          hasMarketNews = true;
          chainStartedToday = true;
          updatedAssetMoods = recordChainRumorMood(
            newChain,
            newDay,
            updatedAssetMoods,
          );
        ...
        if (e) {
          todayNewsItems.push({
            headline: e.headline,
            effects: e.effects,
            labelType: detectLabelType(e.headline, "news"),
          });
          hasMarketNews = true;
          eventFiredToday = e;
    ...
    updatedEscalations = updatedEscalations.filter(
      (esc) => esc.expiresDay > newDay,
    );
    ...
    updatedInvestments.forEach((inv) => {
      if (day + 1 >= inv.resolvesDay) {
        ...
        if (!inv.hintShown && daysInvested >= halfwayPoint && inv.outcome) {
          const startup = [...ANGEL_STARTUPS, ...VC_STARTUPS].find(
            (s) => s.id === inv.startupId,
          );
          if (startup) {
            // Show positive hint if outcome is good, negative if bad
            const isPositive = inv.outcome.multiplier >= 1;
            const hints = isPositive
              ? startup.hints.positive
              : startup.hints.negative;
    ...
    investmentsToResolve.forEach((inv) => {
      if (inv.outcome) {
        ...
        todayNewsItems.push({
          headline: headlineWithAmount,
          effects: inv.outcome.marketEffects || {},
          labelType: "news",
          baseHeadline: inv.outcome.headline,
        });

        if (inv.outcome.marketEffects) {
          Object.entries(inv.outcome.marketEffects).forEach(
            ([assetId, effect]) => {
              effects[assetId] = (effects[assetId] || 0) + effect;
            },
          );
    ...
    if (pendingShitcoin && day + 1 >= pendingShitcoin.resolvesDay) {
      ...
      if (pendingShitcoin.outcome === "moon") {
        ...
        todayNewsItems.push({
          headline:
            "RUG PULLED: $YOURCOIN went to zero. The dev vanished with your $5K.",
          effects: {},
          labelType: "news",
        });
      }
    ...
    if (
      !resolvedShitcoin &&
      currentPendingShitcoin &&
      currentPendingShitcoin.investedDay === newDay
    ) {
      todayNewsItems.push({
        headline:
          "$YOURCOIN IS LIVE: Your meme coin just launched — the market will decide its fate in the coming days...",
        effects: {},
        labelType: "news",
      });
    }
    ...
    if (!hasMarketNews) {
      const unusedQuiet = QUIET_NEWS.filter(
        (q) => !updatedUsedEventHeadlines.includes(q.headline),
      );
      const quietPool = unusedQuiet.length > 0 ? unusedQuiet : QUIET_NEWS;
      const randomQuiet =
        quietPool[Math.floor(Math.random() * quietPool.length)];
      todayNewsItems.push({
        headline: randomQuiet.headline,
        effects: randomQuiet.effects,
        labelType: "news",
      });
      updatedUsedEventHeadlines.push(randomQuiet.headline);
    }

    if (!isScripted && hasMarketNews && Math.random() < 0.3) {
    ...
    LIFESTYLE_ASSETS.forEach((asset) => {
      ...
      const eventEffect =
        (lifestyleEffects[asset.id] || 0) * LIFESTYLE_EFFECT_MULTIPLIER;
      const totalChange = volatilityChange + eventEffect;
      newLifestylePrices[asset.id] = Math.round(
      ...
      todayNewsItems.forEach((newsItem) => {
        Object.keys(newsItem.effects).forEach((assetId) => {
          if (newsItem.effects[assetId] < 0) {
            newsItem.effects[assetId] =
              newsItem.effects[assetId] * (1 - negativeReduction);
          }
        });
      });
    }
    ...
    todayNewsItems.forEach((newsItem) => {
      Object.keys(newsItem.effects).forEach((assetId) => {
        ...
        if (newsItem.effects[assetId] < -0.85)
          newsItem.effects[assetId] = -0.85;
      });
    });
    ...
    ASSETS.forEach((asset) => {
      ...
      newPriceHistory[asset.id] = [
        ...existingHistory,
        {
          ...
          high: Math.round(Math.max(high, open, close) * 100) / 100,
          low:
            Math.round(Math.max(0.01, Math.min(low, open, close)) * 100) / 100,
          close: close,
        },
      ];
    });
    ...
    const startupValue = updatedInvestments.reduce(
    ...
    if (!hasReached1M && nw >= 1000000) {
      todayNewsItems.unshift({
        headline: "🎉 $1M CLUB! VCs will now pitch you their hottest deals.",
        effects: {},
      });
    ...
    const newActiveCelebration =
      newCelebrationQueue.length > 0 ? newCelebrationQueue.shift()! : null;

    let newStartupOffer: typeof get extends () => {
      pendingStartupOffer: infer T;
    }
      ? T
      : never = null;
    ...
    if (isScripted) {
      ...
      if (scriptedDayForStartup?.startupOffer && !hasCelebrations) {
        const startup = selectRandomStartup(
          scriptedDayForStartup.startupOffer.tier,
          updatedUsedStartupIds,
        );
        if (startup) {
          newStartupOffer = startup;
          if (scriptedDayForStartup.startupOffer.tier === "angel")
    ...
    if (!isScripted && !hasCelebrations && newDay <= gameLength - 6) {
      ...
      const daysSinceLastOffer = lastStartupOfferDay
        ? newDay - lastStartupOfferDay
        : Infinity;
      ...
      if (cooldownPassed) {
        ...
        if (nw >= angelNetWorthGate && updatedOfferCounts.angel < angelCap) {
          if (Math.random() < angelBaseChance) {
            const angelOffer = selectRandomStartup(
              "angel",
              updatedUsedStartupIds,
            );
            if (angelOffer) {
    ...
    if (rippleSource) {
      ...
      if (eventSentiment === "bullish" || eventSentiment === "bearish") {
        updatedDirectorState = {
          ...updatedDirectorState,
          secondOrder: applyCounterRipple(
            updatedDirectorState.secondOrder,
            eventSentiment,
          ),
        };
      }
    ...
    if (updatedDirectorState.activeTheme === "none") {
      ...
      const storyStartedToday = updatedStories.find(
        (s) => s.startDay === newDay,
      );
      ...
      if (!seedCategory && eventFiredToday) {
        const totalImpact = Object.values(eventFiredToday.effects).reduce(
          (sum, e) => sum + Math.abs(e),
          0,
        );
      ...
      if (seedCategory) {
        ...
        if (theme) {
          ...
          updatedDirectorState = startTheme(
            updatedDirectorState,
            theme,
            duration,
          );
      ...
      const activeThemeCategory = themeToCategory(
        updatedDirectorState.activeTheme,
      );
      if (activeThemeCategory) {
        const matchingContentFired =
          ...
          updatedStories.some((s) => {
            ...
            return (
              def?.category === activeThemeCategory && s.startDay === newDay
            );
          }) ||
          // Chain started matching theme
          (chainStartedToday &&
            updatedChains.find((c) => c.startDay === newDay)?.category ===
              activeThemeCategory) ||
          // Chain resolved matching theme
          (chainResolvedToday &&
            chainsToResolve.some((c) => c.category === activeThemeCategory)) ||
        ...
        if (matchingContentFired) {
          updatedDirectorState = {
            ...updatedDirectorState,
            themeStrength: Math.min(
              1.0,
              updatedDirectorState.themeStrength + 0.3,
            ),
            themeDaysRemaining: Math.max(
              updatedDirectorState.themeDaysRemaining,
              5,
            ),
          };
        }
    ...
    if (isDev) {
      console.debug(
        `[Chain] STATE day=${newDay}: activeChains=${updatedChains.length}, todayNews=${todayNewsItems.length}, headlines=[${todayNewsItems.map((n) => n.headline.substring(0, 50)).join(" | ")}]`,
      );
    }
    ...
    set({
      ...
      activeBuyMessage: null,
      activeInvestmentBuyMessage: null,
      activeInvestmentResultToast: hasCelebrations
        ? null
        : mostSignificantInvestmentResult,
      activeErrorMessage: null,
      selectedAsset: null,
      ...
      pendingAbilityEffects: clearedPendingEffects
        ? null
        : pendingAbilityEffects,
      ...
      pendingPhase2Effects:
        queuedPhase2Effects ||
        (clearedPhase2Effects ? null : pendingPhase2Effects),
      pendingInflationEffect: clearedInflationEffect
        ? null
        : pendingInflationEffect,
    ...
    if (finalNetWorth < 0 && finalCash <= 0 && !get().pendingGameOver) {
      ...
      if (finalNetWorth < -1_000_000) {
        ...
        shortPositions.length > leveragedPositions.length
      ) {
      ...
      set({
        pendingGameOver: { reason: gameOverReason, netWorth: finalNetWorth },
      });
  ...
  triggerNextDay: () => {
    ...
    const encounterType = rollForEncounter(day + 1, encounterState, cash, {
      netWorth,
  ...
  sell: (assetId: string, qty: number) => {
    ...
    const existingBasis = costBasis[assetId] || { totalCost: 0, totalQty: 0 };
    const avgCost =
      existingBasis.totalQty > 0
        ? existingBasis.totalCost / existingBasis.totalQty
        : price;
  ...
  shortSell: (assetId: string, qty: number) => {
    ...
    const totalShortProceeds = shortPositions.reduce(
      (sum, pos) => sum + pos.cashReceived,
      0,
    );
  ...
  coverShort: (positionId: string) => {
    ...
    const profitLossPct =
      position.cashReceived > 0
        ? (profitLoss / position.cashReceived) * 100
        : 0;
    ...
    appendTradeLog(get, set, {
      ...
      assetName: asset?.name || position.assetId,
      action: "cover_short",
      category: "stock",
  ...
  closeLeveragedPosition: (positionId: string) => {
    ...
    const originalDownPayment =
      (position.qty * position.entryPrice) / position.leverage;
    const profitLoss = afterDebt - originalDownPayment;
    const profitLossPct =
      originalDownPayment > 0 ? (profitLoss / originalDownPayment) * 100 : 0;
  ...
  investInStartup: (amount: number) => {
    ...
    const outcomeBonus = hasPrivateJet ? 0.1 : 0;
    ...
    if (scriptedGameNumber === 1) {
      ...
      const positiveOutcomes = pendingStartupOffer.outcomes.filter(
        (o) => o.multiplier > 0,
      );
      if (positiveOutcomes.length > 0) {
        const baseOutcome =
          positiveOutcomes[Math.floor(Math.random() * positiveOutcomes.length)];
  ...
  cancelStartupLiquidation: () => {
    ...
    if (
      !pendingLiquidation ||
      pendingLiquidation.reason !== "startup_investment"
    )
      return;
  ...
  buyLifestyle: (assetId: string) => {
    ...
    const tradeCategory =
      asset.category === "private_equity" ? "private_equity" : "property";
    const tradeAction =
      asset.category === "private_equity" ? "buy_pe" : "buy_property";
    appendTradeLog(get, set, {
      assetId,
  ...
  sellLifestyle: (assetId: string) => {
    ...
    const sellCategory =
      asset.category === "private_equity" ? "private_equity" : "property";
    const sellAction =
      asset.category === "private_equity" ? "sell_pe" : "sell_property";
    appendTradeLog(get, set, {
      assetId,
      assetName: asset.name,
      action: sellAction,
  ...
  acceptPEExitOffer: () => {
    ...
    const asset = LIFESTYLE_ASSETS.find(
      (a) => a.id === activePEExitOffer.assetId,
    );
    const owned = ownedLifestyle.find(
      (o) => o.assetId === activePEExitOffer.assetId,
    );
    if (!asset || !owned) return;

    const profitLoss = activePEExitOffer.offerAmount - owned.purchasePrice;
    const profitLossPct =
      (activePEExitOffer.offerAmount / owned.purchasePrice - 1) * 100;
    ...
    set({
      ...
      ownedLifestyle: ownedLifestyle.filter(
        (o) => o.assetId !== activePEExitOffer.assetId,
      ),
      activePEExitOffer: null,
      activeSellToast: {
    ...
    appendTradeLog(get, set, {
      assetId: activePEExitOffer.assetId,
      assetName: asset.name,
      action: "pe_exit",
      category: "private_equity",
      quantity: 1,
      price: activePEExitOffer.offerAmount,
      totalValue: activePEExitOffer.offerAmount,
      profitLoss,
    });
  },
  ...
  setShowPortfolio: (show: boolean) =>
    set({
      showPortfolio: show,
      ...(show ? {} : { portfolioAdvancePending: false }),
    }),
  ...
  confirmAdvance: () => {
    ...
    const {
      ...
      isCelebrationDay,
      ownedLuxury,
      activeScript: activeScriptForAdvance,
    } = get();
    const netWorth = get().getNetWorth();
    if (isCelebrationDay) {
      get().nextDay();
  ...
  setActiveActionsTab: (tab: "leverage" | "buy" | "casino") =>
    set({ activeActionsTab: tab }),
  ...
  confirmEncounterResult: (
    result: EncounterResult,
    encounterType: EncounterType,
    pendingShitcoinData?: { outcome: "moon" | "rug"; profit: number } | null,
  ) => {
    const {
      pendingEncounter,
      encounterState,
    ...
    const newEncounterState: EncounterState = {
      ...
      divorceCount:
        encounterState.divorceCount + (encounterType === "divorce" ? 1 : 0),
    ...
    if (
      result.liquidationRequired !== undefined &&
      result.liquidationRequired > 0
    ) {
  ...
  confirmLiquidationSelection: (
    ...
    selectedAssets.forEach((asset) => {
      if (asset.type === "luxury") {
        ...
        newOwnedLifestyle = newOwnedLifestyle.filter(
          (owned) => owned.assetId !== asset.id,
        );
        totalLiquidated += asset.currentValue;
      } else if (asset.type === "leveraged") {
        newLeveragedPositions = newLeveragedPositions.filter(
          (p) => p.id !== asset.id,
        );
        totalLiquidated += asset.currentValue;
  ...
  sellLuxuryAsset: (assetId: LuxuryAssetId) => {
    ...
    const sellPrice = Math.floor(asset.basePrice * 0.8);
  ...
  executePEAbility: (abilityId, peAssetId) => {
    ...
    const {
      PE_ABILITIES,
      getPEAbilities,
    } = require("@/lib/game/lifestyleAssets");
    ...
    if (
      !ability.repeatable &&
      usedPEAbilities.some((u) => u.abilityId === abilityId)
    ) {
    ...
    if (abilityId === "insider_tip") {
      ...
      const scenario =
        INSIDER_TIP_SCENARIOS[
          Math.floor(Math.random() * INSIDER_TIP_SCENARIOS.length)
        ];

      const isUnfavorable = Math.random() < ability.backfireChance;
      const storyOutcome = isUnfavorable ? "unfavorable" : "favorable";
      const effects = isUnfavorable
        ? scenario.backfireEffects.priceEffects || {}
        : scenario.successEffects;

      const part3Headline = isUnfavorable
        ? scenario.headlines.backfirePart3
        : scenario.headlines.successPart3;
    ...
    const effects = isUnfavorable
      ? ability.backfireEffects.priceEffects || {}
      : ability.successEffects;

    const headlines = ABILITY_HEADLINES[abilityId];
    const part1Headline =
      headlines?.part1 || `${ability.name.toUpperCase()} OPERATION UNDERWAY`;
    const part2Headline =
      headlines?.part2 || `${ability.name.toUpperCase()} DEVELOPING...`;
    const part3Headline = isUnfavorable
      ? headlines?.backfirePart3 ||
        `${ability.name.toUpperCase()} OPERATION EXPOSED`
  ...
  confirmElectionResult: () => {
    ...
    if (pendingElection.result === "win") {
      ...
      const currentApexPrice =
        lifestylePrices["pe_apex_media"] || 12_000_000_000;
  ...
  executePresidentialAbility: (abilityId) => {
    ...
    const {
      PRESIDENTIAL_ABILITIES,
    } = require("@/lib/game/presidentialAbilities");
    ...
    if (ability.apexBoost) {
      ...
      updatedLifestylePrices["pe_apex_media"] =
        apexPrice * (1 + ability.apexBoost);
    }
  ...
  getPresidentialAbilityStatus: (abilityId) => {
    ...
    const used = usedPresidentialAbilities.find(
  ...
  getNetWorth: () => {
    ...
    const startupValue = activeInvestments.reduce(
      (sum, inv) => sum + inv.amount,
      0,
    );
  ...
  getTotalDebt: () => {
    ...
    const leverageDebt = leveragedPositions.reduce(
      (sum, pos) => sum + pos.debtAmount,
      0,
    );
